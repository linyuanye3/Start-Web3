...
智能合约、应用协议、质押
### 2025.03.20
今日想搞清楚NFT的价值【每一个区块链都需要发行代币吗？】
### 2025.03.21
接触技术层面，了解ThirdWeb 模板核心功能
1. 模板类型
    NFT Drop：预售或发布NFT集合的落地页（如艺术藏品）。
    Token：代币发行与质押页面。
    DAO：社区治理的引导页（提案、投票、成员展示）。
    Marketplace：NFT交易市场界面。
    Gaming：游戏化任务与奖励系统。
2. 默认模块
    钱包连接：集成 MetaMask、WalletConnect 等主流钱包。
    数据看板：实时显示NFT价格、持有者数量、交易量等。
    交互按钮：Mint NFT、领取代币、加入DAO等CTA。
    响应式设计：适配移动端和桌面端。
3. 技术栈
    前端框架：Next.js + React
