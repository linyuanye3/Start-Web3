...
### 2025.03.17
 去中心化的稳定币和稳定资产
 USDT是一种中心化的稳定币，通过1:1的抵押品系统维持其一美元的挂钩。
 maker是defi在去中心化的“中央银行”的首次尝试。作为一个借贷协议，当抵押品（通常是eth)被存入maker金库时，dai作为借贷需求的副产品被发行。
 尽管dai可能在某种程度上是去中心化的，但它仍然有资本效率低下的问题，无法满足当前的市场需求。此后，出现了一些去中心化的稳定协议，希望改善
 去中心化，价格稳定和资本效率，我们称这些协议为算法稳定币。
 算法稳定币有两种分类：
 1.没有抵押品（esd, ampl和bac)
 2.部分/全部有他们的原生代币抵押（frax, susdhe ust)
 算法稳定币在很大程度上仍然处于试验阶段

### 2025.03.18
 去中心化的永续合约使用户能够在没有到期日的期货合约上建立一个杠杆头村。
 perpetual是一个去中心化的协议，提供永续合约交易。使用独特的虚拟自动做市商（vamms)的方法。
 该协议有自己的本地perp代币，主要作为平台的治理代币。perp代币持有者根据其持有的比例获得投票权，可以在一个固定的时期将他们的perp抵押，以获得更多的perp
 和协议的交易费用的usdc份额。可以去以太坊和xdai主网试试。
 dydx是一个去中心化的交易所协议，提供借贷，现货交易，保证金交易和永续合约交易。在引入layer-2技术和aave和uni等流行代币的更多市场后，dydx可能有潜力
 与perpetual protocal竞争，而且收费较低。

### 2025.03.19
 去中心化期权
 hegic是一个去中心化的链上协议，允许用户购买eth和wbtc的美式看涨和看跌期权。采取流动资金池模式运行。
 有一个流动性挖矿计划，奖励write代币抵押的用户，并根据购买的期权规模和期限奖励期权买家。以hegic代币返还。

 opyn是首批推出的去中心化期权平台之一。让用户以otokens的形式创建代币化的美式期权，锁定100%的基础资产作为抵押。

 这两个期权平台都要由期权卖家进行充分的抵押，但在流动性模式和支持的资产数量方面有很大的不同，都青睐美式期权（随时行权）

### 2025.03.20
  合成资产是指与另一种资产具有相同价值或效果的资产或资产混合物。合成资产跟踪基础资产的价值，并允许在无需持有实际资产本身的情况下接触资产。
  synthetix是一个去中心化的平台，用于铸造和交易被称为synths的合成资产，由平台用户提供的抵押品指出。
  synths是通过过度抵押创造的，这个概念类似于铸造dai的maker.为了铸造synths,用户必须将synthetix代币（snx)作为抵押品。

  uma
  universal market access是一个去中心化的协议，用于在以太坊网络上创建和合成资产。uma使用其技术的两个核心部分提供了构建安全金融合约基础
  设施-- 一个用于构建和部署衍生品的框架，还有一个被称为数据验证机制（dvm)的预言机来执行他们。
  由于合成资产主要依赖预言机作为价格信息的主要来源，虚假的数据可能导致严重的后果。此外，由于合成资产主要通过存入抵押品来铸造，可能会缺乏流动性，
  导致与真实世界的资产相比，定价出现大幅偏差。

  difi的衍生品分支仍然处于起步阶段，总价值很小。
  引导流动性几乎是美一个新兴的defi项目面临的问题，即使有激励措施，这些产品具有较高的波动性。


### 2025.03.21
  去中心化保险
  拥有一个强大的保险系统是减少用户在与defi应用互动时承担风险的关键措施，从而吸引更多用户。defi需要保险产品以便让拥有大量资金部署的机构相信参与defi是安全的。

  nexus mutual是加密货币市场上最大的defi保险协议，提供两种类型的保险：
  1.协议保障（protocal covers)
  覆盖保管用户资金的defi协议，协议保险以前被称为智能合约保险，包括：
   经济设计失败
   严重的预言机失败
   治理攻击
   对第二层资产的保护
  ...
  要从nexus mutual购买保险，用户首先要通过kyc注册成为互助成员。一旦获得批准，用户就可以使用eth或dai购买保险。
