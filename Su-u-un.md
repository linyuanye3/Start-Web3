...
- DeFi(去中心化金融):建立在区块链上的数字资产、智能合约、协议、去中心化应用
- 不同于CeFi(去中心化交易所)，你干了啥事都写在区块链上面，而不是交易所的数据库。
- DeFi包含:交易所、稳定币、存借贷、理财、金融衍生品、支付、期货合约、跨链支付、预言机(外部数据输入到链上的作用)、彩票、拍卖、裁判等等
- 收益来源:利息、币价波动、理财、意外之财(彩票、拍卖)
...
除DeFi和nft之外。还有很多相关的web3平台、产品、服务。

- 区块链网络：比特币、以太坊等
- 跨链桥：实现数字资产和数据在不同链中的传输和交互，across protocol；synapse protocol；stargate
- 节点服务：infura（连接钱包时候
- 网络存储：ipfs（展示nft图片时候
- 数据服务：etherscan以太坊浏览器
- 智能合约
- 质押

#### 黑暗森林1 钓鱼攻击
- 钓鱼攻击1.0：转账和转nft.因为太明显，进化到2.0
- 钓鱼攻击2.0：授权额度，点两下就转钱了---进化---提升额度，点两下转钱。
- 如何防范：警惕链接，使用安全工具，风险隔离（鸡蛋不在一个篮子）

#### 黑暗森林2 恶意签名攻击
- 模仿常用的网站登陆时的签名请求，如果签名了恶意网站，资金将会受损。
- 相当于你和别人签了合同，允许他帮你做交易。然后黑客用假的合同给你签。
...
#### 黑暗森林4 虚假地址攻击
- 多数应用会缩略地址显示，当你在进行交易时，骗子使用首尾相似的地址达成欺骗。
- 存在剪贴板木马，会修改剪贴板，导致你的交易对象被篡改。
- 通过发送小额历史交易，成为交易对象，被软件识别为信任对象。
- 防御方法：检查地址，检查粘贴文本，标记信任对象。
...
- 低级貔貅盘：可以买入不可卖出
- 进阶貔貅盘：超过某个限制不可卖出
- 貔貅盘原理：设置交易规则、白名单黑名单等手段，使买家无法出售。
- 鉴别貔貅盘：检查合约是否开源，远离不开源合约，仔细阅读开源合约；使用新地址购买，小酌怡情

### 2025.03.23

- 铭文是区块链上刻写的数据，如图片、文字、代码等
- 起源于比特币里面的聪单位，有的聪很特别，可以把一些数据存在这些聪上面，这样就得到了独一无二的一个铭文
- 铭文不像代币一样可以拆分，他不作为交易手段，更多是作为艺术和数据存储
